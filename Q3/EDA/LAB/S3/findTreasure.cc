...
bool findTreasure(const map_graf &map, const coord_2d &map_size, coord_2d curr_pos, visited_matrix &visited) {
   if (!visited[curr_pos.height - 1][curr_pos.width - 1] && map[curr_pos.height - 1][curr_pos.width - 1] != 'X') {
      ...
      if (map[curr_pos.height - 1][curr_pos.width - 1] == 't') return true;
      bool found = false;
      for (int i = -1; i <= 1; ++i) {
         for (int j = -1; j <= 1; ++j) {
            if (i == 0 or j == 0) { // you cannot move in diagonal.
               ...
               if (y_limits && x_limits && !found) {
                  found = findTreasure(map, map_size, next_pos, visited);
               }
            }
         }
      }
...
bool findTreasure(const map_graf &map, const coord_2d &map_size, coord_2d ini_pos) {
   vector<vector<bool> > visited(map_size.height, vector<bool>(map_size.width, false));
   return findTreasure(map, map_size, ini_pos, visited);
}
...
int main() {
   ...
   findTreasure(map, map_size, ini_pos) ? cout << "yes" : cout << "no";
   cout << endl;
}