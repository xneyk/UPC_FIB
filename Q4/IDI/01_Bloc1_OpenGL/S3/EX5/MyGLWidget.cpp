...
void MyGLWidget::mousePressEvent(QMouseEvent *e) {
   if (e->button() == Qt::LeftButton) {
      oldMouseX = e->x(); // Captura la posició actual del mouse abans de començar a moure'l
      oldMouseY = e->y();
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e) {
   if (mouseBeingPressed) {
      if (e->x() - oldMouseX > 0) sx += 0.01f;
      else if (e->x() - oldMouseX < 0) sx -= 0.01f;
      
      if (e->y() - oldMouseY > 0) sy -= 0.01f;
      else if (e->y() - oldMouseY < 0) sy += 0.01f;
   }
...
void MyGLWidget::modelTransform() {
   ...
   /*1*/ TG = glm::scale(TG, glm::vec3(sx, sy, 1.0f)); // Escalat en funció del parametre amb KeyEvents
