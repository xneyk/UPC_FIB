...
void MyGLWidget::paintGL() {
   ...
   // Activem el VAO per a pintar a patricio
   glBindVertexArray(patricio.VAO);
   
   // Carreguem la transformació de model per l'objecte 3
   patricio1ModelTransform();
   // pintem
   glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat)*patricio.model.faces().size()*3*3);

   // Carreguem la transformació de model per l'objecte 3
   patricio2ModelTransform();
   // pintem
   glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat)*patricio.model.faces().size()*3*3);

   // Carreguem la transformació de model per l'objecte 3
   patricio3ModelTransform();
   // pintem
   glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat)*patricio.model.faces().size()*3*3);

   // Carreguem la transformació de model per l'objecte 4`
   patricio4ModelTransform();
   // pintem
   glDrawArrays(GL_TRIANGLES, 0, sizeof(GLfloat)*patricio.model.faces().size()*3*3);
...
void MyGLWidget::resizeGL(int w, int h) {
   ...
   if (ra < 1) {
      // ajustem el nou FOV en cas que hagi augmentat en vertical
...
void MyGLWidget::keyPressEvent(QKeyEvent *e) {
   ...
   switch (e->key()) {
      case Qt::Key_O:
         ...
         break;
      case Qt::Key_Z:
         prespective_zoom_factor += 0.5f;
         ortho_zoom_factor += 0.1f;
         projectTransform();
         break;
      case Qt::Key_X:
         prespective_zoom_factor -= 0.5f;
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e) {
   ...
   if (mouseBeingPressed) {
      if (e->x() - old_x > 0) angle_x += 0.01f;
      else if (e->x() - old_x < 0) angle_x -= 0.01f;

      if (e->y() - old_y > 0) angle_y += 0.01f;
      else if (e->y() - old_y < 0) angle_y -= 0.01f;
   }
   old_x = e->x();
   old_y = e->y();
   viewTransform();
...
void MyGLWidget::patricio1ModelTransform() {
   glm::mat4 TG(1.0f);
   TG = glm::translate(TG, glm::vec3(2.0, 1.0/2.0, 2.0));
...
void MyGLWidget::patricio2ModelTransform() {
   glm::mat4 TG(1.0f);
   TG = glm::translate(TG, glm::vec3(0.0, 1.0/2.0, 0.0));
   TG = glm::scale(TG, glm::vec3(1.0/patricio.height)); // escalat uniforme
...
void MyGLWidget::patricio3ModelTransform() {
   glm::mat4 TG(1.0f);
   TG = glm::translate(TG, glm::vec3(-2.0, 1.0/2.0, -2.0));
   TG = glm::scale(TG, glm::vec3(1.0/patricio.height)); // escalat uniforme
...
void MyGLWidget::viewTransform() {
   ...
   // Angles d'Euler
   glm::mat4 VM(1.0);
   VM = glm::translate(VM, glm::vec3(0.0, 0.0, -D)); // l'emputjem cap a Z negatives (al front de la càmera).
   VM = glm::rotate(VM, angle_z, glm::vec3(0.0, 0.0, 1.0));
   VM = glm::rotate(VM, angle_y, glm::vec3(1.0, 0.0, 0.0));
   VM = glm::rotate(VM, angle_x, glm::vec3(0.0, 1.0, 0.0));
...
void MyGLWidget::projectTransform() {
   glm::mat4 PM(1.0);
   if (isPrespective) {
      PM = glm::perspective(FOV+prespective_zoom_factor, ra, zNear, zFar);
   } else {
      PM = glm::ortho(
         l * ortho_zoom_factor,
         r * ortho_zoom_factor,
...
void MyGLWidget::iniCamera() {
   ...
   setSphere(glm::vec3(-2.5, 0.0, -2.5), glm::vec3(2.5, 1.0, 2.5));

   D = 3*radius;
   VRP = center;
   viewTransform(); // Enviem la VM al VS
   // Parametres càmera prespectiva
   prespective_zoom_factor = 0.0f;
   ortho_zoom_factor = 1.0f;
   FOV_ini = FOV = 2 * glm::asin(radius/D);
   ra = 1.0f;
   zNear = D - radius;
   zFar = D + radius;
   // Parametres càmera ortogonal
   l = b = -radius;
   r = t = radius;
