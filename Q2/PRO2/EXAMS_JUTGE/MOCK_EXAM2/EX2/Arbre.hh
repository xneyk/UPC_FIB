...
class Arbre {
  ...
  public:
   ...
   Arbre suma(const Arbre& a) const {
      Arbre r = Arbre();
      r.primer_node = sum_aux(primer_node, a.primer_node);
      return r;
   }

  private:
   static node_arbre* sum_aux(node_arbre* n,  node_arbre *m){
      node_arbre *sum = new node_arbre;
      if (n != nullptr and m != nullptr) {
         sum->info = n->info + m->info;
         sum->segE = sum_aux(n->segE, m->segE);
         sum->segD = sum_aux(n->segD, m->segD);
