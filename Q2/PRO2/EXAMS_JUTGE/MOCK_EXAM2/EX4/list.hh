   ...
   class iterator {
     ...
     public:
         ...
         el resultat és el p.i. */
      {
         ...
         if (linked_with != nullptr) { // if its linked to an iterator
            if (linked_with->pitem != &(linked_with->plist->itemsup)) { // if it can advance
               linked_with->pitem = linked_with->pitem->next;
            }
         }
         return *this;
      }
         ...
         el resultat és el p.i. */
      {
         ...
         if (linked_with != nullptr) { // if its linked to an iterator
            if (linked_with->pitem != linked_with->plist->iteminf.next) { // if can go back
               linked_with->pitem = linked_with->pitem->prev;
            }
         }
         return *this;
      }
