...
class Arbre {
  ...
  public:
   ...
   bool evaluate() const {
      return primer_node != nullptr ? evaluate(primer_node) : false;
   }

  private:
   static bool evaluate(node_arbre* n) {
      if (n->info == "not") return not evaluate(n->segE);
      if (n->info == "or") return evaluate(n->segE) or evaluate(n->segD);
      if (n->info == "and") return evaluate(n->segE) and evaluate(n->segD);
      
      return n->info == "true"; // other case n->value == "false"
   }
};
