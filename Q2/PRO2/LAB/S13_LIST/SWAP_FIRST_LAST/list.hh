...
class List {
  ...
  public:
   ...
   void swapFirstLast() {
      if (_size == 2){ // Edge case... swap two adjacent nodes.
         Item *new_last = iteminf.next; // new last is old first
         Item *new_first = itemsup.prev; // new first is old last
         
         iteminf.next = new_first;
         itemsup.prev = new_last;
         new_first->prev = &iteminf;
         new_first->next = new_last;
         new_last->prev = new_first;
         new_last->next = &itemsup;
      } 
      else if (_size > 2) {
         Item *new_last = iteminf.next; // new last is old first
         Item *new_first = itemsup.prev; // new first is old last
         Item *second_first = iteminf.next->next; 
         Item *second_last = itemsup.prev->prev;
         // need to fix neighboring nodes.
         second_first->prev = new_first;
         second_last->next = new_last;
         iteminf.next = new_first;
         itemsup.prev = new_last;
         // Connect swaping nodes to which it corresponds.
         new_last->prev = second_last;
         new_last->next = &itemsup;
