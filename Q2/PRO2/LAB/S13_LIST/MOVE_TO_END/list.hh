...
class List {
  ...
  public:
   ...
   void moveToEnd(iterator &it) {
      if (it.pitem != itemsup.prev) { // ==> _size >= 2
         Item *left = it.pitem->prev;
         Item *right = it.pitem->next;

         // Deconnect item pointed by "it".
         left->next = it.pitem->next;
         right->prev = it.pitem->prev;
         // Reconnect it at the end of the list.
         Item *neighbour = itemsup.prev;
         
         neighbour->next = it.pitem;
         it.pitem->prev = neighbour;
