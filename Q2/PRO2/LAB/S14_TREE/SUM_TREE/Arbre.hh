  ...
  public:
   ...
   void fills(Arbre& fe, Arbre& fd) {
      ...
      /* Post: fe �s el fill esquerre d'A, fd �s el fill dret d'A,
         el p.i. �s buit */
      if (primer_node != NULL and fe.primer_node == NULL and fd.primer_node == NULL) {
         if (&fe != &fd) {
            node_arbre* aux;
            aux = primer_node;
   ...
   /* Pre: cert */
   /* Post: el paràmetre impícit ara és el seu arbre de sumes */
   static node_arbre* build_node(node_arbre *node) {
      if (node != nullptr) {
         node_arbre *result = new node_arbre;
         
         result->segE = build_node(node->segE);
         result->segD = build_node(node->segD);
         result->info = node->info;
         
         if (result->segE != nullptr) result->info += result->segE->info;
         if (result->segD != nullptr) result->info += result->segD->info;
