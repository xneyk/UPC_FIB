...
public:
  ...
  void pop2() {
    if (ptopitem != nullptr) {
      if (ptopitem->next != nullptr) { // Exists a 2nd top element => delete it
        Item *new_second_top = ptopitem->next->next;
        delete ptopitem->next;
        ptopitem->next = new_second_top;
      } else { // There only exists one element => delete it
        delete ptopitem;
        ptopitem = nullptr;
      }
      --_size;
