...
{
   int equal_h, equal_v;
   tareador_disable_object(&X11_COLOR_fake); // disable global variable for not counting it as a dependency
   tareador_disable_object(histogram);       // disable global variable histogram for not counting it as a dependency   
   
   for (int y = 0; y < ROWS; y += TILE) {
      for (int x = 0; x < COLS; x += TILE) {
         //check horizontal borders
         tareador_start_task("mandle_check_horizontal");
         equal_h = 1;
         for (int px = x; px < x + TILE; px++) {
            ...
            equal_h = equal_h & (M[y][x] == M[y][px]);
            equal_h = equal_h & (M[y][x] == M[y + TILE - 1][px]);
         }
         tareador_end_task("mandle_check_horizontal");
         //check vertical borders
         tareador_start_task("mandle_check_vertical");
         equal_v = 1;
         for (int py = y; py < y + TILE; py++) {
            ...
            equal_v = equal_v & (M[y][x] == M[py][x]);
            equal_v = equal_v & (M[y][x] == M[py][x + TILE - 1]);
         }
         tareador_end_task("mandle_check_vertical");         

         //check if we can avoid computation of tile
         tareador_start_task("mandle_check_or_compute_tile");
         if (equal_h & equal_v) {
            if (output2histogram)
            ...
            for (int py = y; py < y + TILE; py++) {
               tareador_start_task("mandle_fill_py_tile");
               for (int px = x; px < x + TILE; px++) {
                  M[py][px] = M[y][x];
                  if (output2display) {
               ...
               tareador_end_task("mandle_fill_py_tile");
            }
         } else {
            // computation of tile
            for (int py = y; py < y + TILE; py++) {
               tareador_start_task("mandle_compute_py_tile");
               for (int px = x; px < x + TILE; px++) {
               ...
               tareador_end_task("mandle_compute_py_tile");
            }
         }
         tareador_end_task("mandle_check_or_compute_tile");
      }
   }
}
