...
void mandel_simple(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
   ...
   #pragma omp parallel
   {
      ...
      int start = id_thread*BS;
      int end; 
      int mod = COLS%omp_get_num_threads(); // nombre de columnes que poden haver quedat pendents de repartir.
      if (id_thread < mod) {
         start += id_thread;
         // id_0 se li suma 0
         // id_1 se li suma el desplaçament d'haver afegit 1 a id_0 (1)
         // id_2 se li suma el desplaçament d'hacer afegit 1 a id_0 i 1 a id_1 (2)
         // and so on...
         end = start + BS + 1; // li afegim 1 element
      } else {
         start += mod; // tots els que quedaven pendents de repartir han estat assignats -> afegim l'offset.   
         end = start + BS;
      }
