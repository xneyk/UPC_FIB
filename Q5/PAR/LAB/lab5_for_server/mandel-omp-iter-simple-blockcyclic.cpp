...
#define CACHE_LINE 64          // En la arquitectura on sera executat, el tamany de linea de cache es de 4.
...
void mandel_simple(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
   #pragma omp parallel
   {
      ...
      int block_size = CACHE_LINE/sizeof(int);
      // ja que el que tractem son enters, i volem que cada block sigui una linea de cache.
      // C_L bytes/line / 4 bytes/word ==> words/line       

      // Calcular
      for (int y = 0; y < ROWS; y++) {
         for (int x = id_thread*block_size; x < COLS; x += n_threads*block_size) {
               for (int xx = x; xx < std::min(x + block_size, COLS); xx++) {
               M[y][xx] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI, xx, y, scale_real, scale_imag, maxiter);
               if (output2histogram)
                  #pragma omp atomic
                  histogram[M[y][xx] - 1]++;
               if (output2display) {
                  /* Scale color and display point  */
                  long color = (long)((M[y][xx] - 1) * scale_color) + min_color;
                  if (setup_return == EXIT_SUCCESS) {
                     #pragma omp critical
                     {
                        XSetForeground(display, gc, color);
                        XDrawPoint(display, win, gc, xx, y);
                     }
                  }
               }
            }
